...
export default function NewsCard({ post, featured }: NewsCardProps) {
  ...
  return (
    <Link
      href={`/news/${post.slug}`}
      className="group relative flex flex-col overflow-hidden bg-[#E4DCBE] rounded-xl card-lift"
    ...
    >
      ...
      <div className="relative w-full h-44 overflow-hidden rounded-t-xl photo-placeholder">
        ...
        <div className="absolute top-3 right-3">
          <time
            className="text-[#6B7A99] text-xs bg-[#E4DCBE]/80 backdrop-blur-sm px-2 py-0.5 rounded-full"
            dateTime={post.date}
            style={{ fontFamily: "var(--font-cormorant)" }}
          >
            {post.date}
      ...
      {/* ── テキストエリア ── */}
      <div className="flex-1 p-5 bg-[#E4DCBE] rounded-b-xl">
        <h3
