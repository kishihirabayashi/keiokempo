...
const resultBadgeColors: Record<string, string> = {
  ...
  "ベスト8": "bg-[#D7CEA9] text-[#6B7A99] border-[#D4C9B8]",
  "ベスト16": "bg-[#D7CEA9] text-[#A0AAB8] border-[#D4C9B8]",
};
...
function getResultBadge(result: string) {
  ...
  return "bg-[#D7CEA9] text-[#6B7A99] border-[#D4C9B8]";
}
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {filtered.map((r, i) => (
            <tr
              key={i}
              className="border-b border-[#D4C9B8] hover:bg-[#D7CEA9] transition-colors duration-300"
            >
              <td
